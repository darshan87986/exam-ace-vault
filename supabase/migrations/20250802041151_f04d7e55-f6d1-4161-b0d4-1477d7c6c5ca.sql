...
-- Add subject_id to Exam-prep table
...
-- Insert some sample subjects for BCA 1st semester
...
-- Insert some sample subjects for BBA 1st semester
INSERT INTO public.subjects (semester_id, name, code, description)
SELECT s.id, sub.name, sub.code, sub.description
FROM public.semesters s
JOIN public.degrees d ON s.degree_id = d.id,
...
-- Allow authenticated users to manage semesters and subjects
